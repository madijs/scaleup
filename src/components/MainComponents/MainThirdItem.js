...
import MainSecondItem from "./MainSecondItem";
import cashFormat from "../../tools/cashFormat";

const MainThirdItem = ({openModal,info,disabled}) => {
    return(
        <div style={{height:'940px'}} className={styles.block}>
            <div className={styles.block_pad}>
            <div className={styles.block_title}>
                {info.name}
            </div>
            <div className={styles.block_price}>
                {cashFormat(info.price)}            </div>
